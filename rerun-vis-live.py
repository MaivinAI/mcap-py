...
def visualizer(ip_addr, image_scaling, memory_limit):
    ...
    try:
        ...
        def pc2_listener(message):
            radar_data = PointCloud2.deserialize(message.value.payload)
            radar_points = []
        ...
        cfg = zenoh.Config()
        cfg.insert_json5(zenoh.config.CONNECT_KEY, '["tcp/' + ip_addr + '"]')
...
def main():
    ...
    visualizer(opt.connect, opt.scale, opt.memory)
